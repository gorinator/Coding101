...
int main()
{
	...
	time_t c = time(0);//создаем переменную 'c' с текущим временем в секундах

	getline(cin, text);//вводим и считываем из потока текст, и заносим его в меременную text

	time_t d = time(0);//создаем переменную 'd' с текущим временем в секундах

	e = (float)d - c;//вычисляем сколько секунд прошло с момента начала написания текста
	...
	cout << "Vi pechataete so skorostiu-" << (float)f << " simvolod v minutu.";//выводим результат в консоль.
